...
- **AST Filtering:** Only the modulo (`%`) binary operator is permitted; all other binary operations are blocked. However, unary operations (like `-~x`) are allowed.
...
- **Uppercase Letters:** All uppercase letters are allowed, enabling variable names and string construction.
...
- **Format Strings:** The `%` operator allows constructing single characters: `'%c' % 65` yields `'A'`.
...
##### Step 2: Build Arbitrary Integers
...
##### Step 3: Build Arbitrary Strings
...
##### Step 4: Dynamic Attribute Access
...
##### Step 5: Chain Attribute Access to Reach `os.system`
...
##### Step 6: Assemble the Final Payload
...
# ————————————————————————————————————————————————————————————————
# 2. Spawn local stub, feed it the payload, and dump stdout+stderr
# ————————————————————————————————————————————————————————————————
